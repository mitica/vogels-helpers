...
Record.prototype.normalize = function() {
	...
	if (this.isCreating()) {
		if (this.config.createNormalize) {
			data = this.config.createNormalize(data);
		}
		schema = this.config.createSchema;
	} else {
		if (this.config.updateNormalize) {
			data = this.config.updateNormalize(data);
		}
		schema = this.config.updateSchema || this.config.createSchema;
	}
...
Record.prototype.validate = function() {
	...
	var schema;
	if (this.isCreating()) {
		if (this.config.createValidate) {
			this.config.createValidate(data);
		}
		schema = this.config.createSchema;
	} else {
		if (this.config.updateValidate) {
			this.config.updateValidate(data);
		}
